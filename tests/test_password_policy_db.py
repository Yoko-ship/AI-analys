...
def _fail(store, times):
    for _ in range(times):
        with pytest.raises(ValueError):
            store.accounts.login_user(EMAIL, "wrong-password-9")
...
def test_weak_passwords_are_refused_by_every_setter(store):
    with pytest.raises(password_policy.WeakPassword):
        store.accounts.register_user(EMAIL, "12345678")
    with pytest.raises(password_policy.WeakPassword):
        store.accounts.start_registration(EMAIL, "password1")
    store.accounts.register_user(EMAIL, GOOD, "Azamxon Gapparov")
    code = store.accounts.issue_email_code(EMAIL, "reset")
    with pytest.raises(password_policy.WeakPassword):
        store.accounts.reset_password_with_code(EMAIL, code, "qwertyuiop")
    with pytest.raises(password_policy.WeakPassword) as exc:
        store.security.change_password(_column(store, "id"), GOOD, "azamxon-2026", "no-token")
...
def test_login_upgrades_an_old_hash(store):
    store.accounts.register_user(EMAIL, GOOD)
    with store.database.connect() as conn:
        conn.execute("UPDATE web_users SET password_hash = %s WHERE email = %s",
                     (identity_credentials._password_hash(GOOD, iterations=210_000), EMAIL))
    assert _column(store, "password_hash").split("$")[1] == "210000"
    store.accounts.login_user(EMAIL, GOOD)
    assert _column(store, "password_hash").split("$")[1] == str(identity_settings.PBKDF2_ITERATIONS)
    store.accounts.login_user(EMAIL, GOOD)  # the upgraded hash still verifies
...
def test_ten_wrong_passwords_lock_the_account(store):
    store.accounts.register_user(EMAIL, GOOD)
    _fail(store, identity_settings.LOGIN_FAILURE_LIMIT - 1)
    with pytest.raises(identity_users.AccountLocked) as exc:
        store.accounts.login_user(EMAIL, "wrong-password-9")
    assert exc.value.newly_locked
    assert 0 < exc.value.retry_after <= identity_settings.LOGIN_LOCK_MINUTES * 60
    # While locked even the right password is refused, and no new notice is due.
    with pytest.raises(identity_users.AccountLocked) as again:
        store.accounts.login_user(EMAIL, GOOD)
...
def test_the_lock_expires_and_a_success_clears_the_counter(store):
    store.accounts.register_user(EMAIL, GOOD)
    _fail(store, identity_settings.LOGIN_FAILURE_LIMIT - 1)
    with pytest.raises(identity_users.AccountLocked):
        store.accounts.login_user(EMAIL, "wrong-password-9")
    with store.database.connect() as conn:
    ...
    _fail(store, identity_settings.LOGIN_FAILURE_LIMIT - 1)  # a fresh budget, not one attempt left
...
def test_password_reset_unlocks_the_account(store):
    ...
    with pytest.raises(identity_users.AccountLocked):
        store.accounts.login_user(EMAIL, "wrong-password-9")
    code = store.accounts.issue_email_code(EMAIL, "reset")
    store.accounts.reset_password_with_code(EMAIL, code, "Violin-Lamp-Tree-77")
    store.accounts.login_user(EMAIL, "Violin-Lamp-Tree-77")


def test_the_lock_notice_uses_the_owners_language(store):
    store.accounts.register_user(EMAIL, GOOD)
    store.profile.update_preferences(_column(store, "id"), {"language": "uz"})
    _fail(store, identity_settings.LOGIN_FAILURE_LIMIT - 1)
    with pytest.raises(identity_users.AccountLocked) as exc:
        store.accounts.login_user(EMAIL, "wrong-password-9")
...
def test_unknown_addresses_fail_like_wrong_passwords(store):
    with pytest.raises(ValueError, match="Invalid email or password"):
        store.accounts.login_user("nobody@example.test", "wrong-password-9")
