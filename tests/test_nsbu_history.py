...
_HARVEST = rc.harvest_historical_quarters
...
def catalog(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(rc, "harvest_historical_quarters", lambda ticker, **kw: {"added": 0, "errors": []})
    ...
    monkeypatch.setattr(rc, "compute_financial_ratios", lambda *a: {"source_values": dict(values), "metrics": {"ROA": 1.0}})
    calls = []
    ...
    monkeypatch.setattr(rc, "fetch_report_excel_data", fetch)
    monkeypatch.setattr(rc, "_register_parse", lambda *a: 42)
    return calls, values
...
def report(ticker="UZIR", year=2018, quarter=3, url="https://openinfo.uz/42.xlsx", record="42", form="NSBU", published="2018-11-01"):
    c = rc.get_catalog_conn()
    with c:
        rc._upsert_report(c, ticker, report_form=form, period_type="quarter" if quarter else "annual",
...
def test_all_periods_and_aliases_resume_and_msfo_is_untouched(catalog):
    ...
    report(quarter=0)
    report(form="MSFO")
    rc.upsert_financials_cache("UZIRP", "NSBU", 2018, 3, {"revenue": 1})
...
def test_changed_url_record_parser_and_lost_cache_require_retry(catalog, monkeypatch):
    ...
    assert rc.get_report_urls("UZIRP", "NSBU", 2018, 3)["excel_url"].endswith("new.xlsx")
...
def test_partial_and_failed_periods_retry_without_stopping_other_periods(catalog, monkeypatch):
    ...
    values["cash"] = None
    original = rc.fetch_report_excel_data
...
def test_budget_future_periods_and_discovery_failure(catalog, monkeypatch):
    ...
    monkeypatch.setattr(rc, "harvest_historical_quarters", fail)
...
def test_incomplete_parse_preserves_published_fields_and_nonfinite_is_rejected(catalog):
    report()
    rc.upsert_financials_cache("UZIRP", "NSBU", 2018, 3, {"cash": 10, "revenue": 20})
    calls, values = catalog
    values["cash"] = None
    result = history.run()
...
def test_discovery_cannot_publish_before_validation(catalog, monkeypatch):
    monkeypatch.setattr(rc, "unified_quarterly_records", lambda *a: [{
        "accounting_id": "42", "pdf_id": "123", "pub_date": "2018-11-01",
        "excel_url": "https://openinfo.uz/42.xlsx", "title": "Q3",
    }])
    monkeypatch.setattr(rc, "_parse_workbook_uncached", lambda *a: {"ok": True})
    monkeypatch.setattr(rc, "compute_financial_ratios", lambda *a: {"source_values": {"revenue": 1}})
    monkeypatch.setattr(rc, "_register_parse", lambda *a: pytest.fail("discovery published provenance"))
    rc.upsert_financials_cache("UZIR", "NSBU", 2018, 3, {"revenue": 100, "cash": 10})
    ...
    c.close()
    assert rc.get_report_urls("UZIR", "NSBU", 2018, 3)["excel_url"].endswith("42.xlsx")


def test_runner_discovery_is_catalog_only(catalog, monkeypatch):
    calls = []
    monkeypatch.setattr(rc, "harvest_historical_quarters", lambda *a, **kw: calls.append(kw) or {})
    history.run()
...
def test_bank_optional_fields_do_not_invalidate_successful_checkpoint(catalog):
    ...
    rc.upsert_financials_cache("UZIR", "NSBU", 2018, 3, {"current_assets": 50})
    assert history.run()["complete"] == 1
