...
def setup(monkeypatch):
    ...
    monkeypatch.setattr(subject_web_auth.web_auth_store.sessions, "get_user_by_token", lambda token: SimpleNamespace(id=41, email="admin@example.com"))
...
def test_non_admin_cannot_access(setup, monkeypatch):
    client, query, _ = setup
    monkeypatch.setattr(subject_web_auth.web_auth_store.sessions, "get_user_by_token", lambda token: SimpleNamespace(id=7, email="reader@example.com"))
