...
def test_new_hashes_use_600k_rounds_and_old_ones_still_verify():
    assert identity_settings.PBKDF2_ITERATIONS >= 600_000
    fresh = identity_credentials._password_hash("correct horse battery staple")
    assert fresh.split("$")[1] == str(identity_settings.PBKDF2_ITERATIONS)
    assert identity_credentials._password_verify("correct horse battery staple", fresh)
    old = identity_credentials._password_hash("correct horse battery staple", iterations=210_000)
    assert identity_credentials._password_verify("correct horse battery staple", old)
    assert identity_credentials.password_needs_rehash(old)
    assert not identity_credentials.password_needs_rehash(fresh)
...
def test_register_refuses_a_weak_password_before_touching_the_store(client, monkeypatch):
    monkeypatch.setattr(subject_web_auth.web_auth_store.accounts, "start_registration",
...
def test_password_reset_refuses_a_weak_password(client, monkeypatch):
    monkeypatch.setattr(subject_web_auth.web_auth_store.accounts, "reset_password_with_code",
...
def test_locked_login_is_429_with_retry_after_and_notifies_the_owner_once(client, monkeypatch):
    ...
    def locked(*args, **kwargs):
        raise identity_users.AccountLocked("reader@example.uz", retry_after=900, newly_locked=True, language="uz")
    monkeypatch.setattr(subject_web_auth.web_auth_store.accounts, "login_user", locked)
    ...
    def still_locked(*args, **kwargs):
        raise identity_users.AccountLocked("reader@example.uz", retry_after=600, newly_locked=False, language="uz")
    monkeypatch.setattr(subject_web_auth.web_auth_store.accounts, "login_user", still_locked)
