...
class TestPeriodKey:
    ...
    def test_keys(self, period, expected) -> None:
        assert catalogue_periods._period_key(period) == expected

    def test_an_annual_outranks_its_own_quarters(self) -> None:
        assert catalogue_periods._period_key("2024") > catalogue_periods._period_key("2024Q4")

    def test_a_later_year_outranks_an_earlier_annual(self) -> None:
        assert catalogue_periods._period_key("2025Q1") > catalogue_periods._period_key("2024")

    def test_corrupt_periods_lose_to_everything_real(self) -> None:
        # '2025Q7' > '2025Q3' lexicographically, which is how it used to win.
        assert catalogue_periods._period_key("2025Q7") < catalogue_periods._period_key("2015Q1")


class TestPrematureAnnuals:
    def test_the_current_year_annual_is_premature(self) -> None:
        current = datetime.now(timezone.utc).year
        assert catalogue_periods._is_premature_annual_period(str(current)) is True
        assert catalogue_periods._is_premature_annual_year(current) is True

    def test_last_completed_year_is_not(self) -> None:
        assert catalogue_periods._is_premature_annual_period(str(catalogue_periods._latest_complete_fiscal_year())) is False
    ...
    def test_a_current_year_quarter_stays_valid(self) -> None:
        ...
        current = datetime.now(timezone.utc).year
        assert catalogue_periods._is_premature_annual_period(f"{current}Q1") is False

    def test_a_premature_annual_ranks_below_every_real_period(self) -> None:
        current = datetime.now(timezone.utc).year
        assert catalogue_periods._fact_period_rank(str(current)) < catalogue_periods._fact_period_rank("2015Q1")

    def test_but_still_above_junk(self) -> None:
        # An issuer whose ONLY datum is the placeholder should show it, not nothing.
        current = datetime.now(timezone.utc).year
        assert catalogue_periods._fact_period_rank(str(current)) > catalogue_periods._fact_period_rank("2025Q7")

    def test_completed_annual_yields_to_a_newer_quarter_after_freshness_window(self, monkeypatch) -> None:
        monkeypatch.setattr(catalogue_settings, "FINANCIALS_ANNUAL_FRESH_DAYS", 210)
        last_fy = catalogue_periods._latest_complete_fiscal_year()

        assert catalogue_periods._completed_annual_is_recent(last_fy, today=date(last_fy + 1, 6, 30))
        assert not catalogue_periods._completed_annual_is_recent(last_fy, today=date(last_fy + 1, 9, 10))
...
class TestFinancialsPeriodSelectionSql:
    ...
    def test_a_completed_annual_beats_the_same_years_q1(self, catalog) -> None:
        ...
        got = catalogue_snapshots.get_all_financials()
    ...
    def test_is_ytd_flags_a_cumulative_quarter(self, catalog) -> None:
        ...
        assert catalogue_snapshots.get_all_financials()["FFF"]["is_ytd"] is True

    def test_is_ytd_is_false_for_an_annual(self, catalog) -> None:
        last_fy = catalogue_periods._latest_complete_fiscal_year()
        self._insert(catalog, "GGG", last_fy, 0, 400.0)
        assert catalogue_snapshots.get_all_financials()["GGG"]["is_ytd"] is False
...
class TestFuturePeriodsAreRefusedAtTheWriteBoundary:
    ...
    def test_a_quarter_that_has_not_ended_is_rejected(self, catalog) -> None:
        current = datetime.now(timezone.utc).year
        assert catalogue_financial_store.bulk_replace_financials([self._row("AAA", current, 4, 999.0)]) == 0
        assert catalogue_snapshots.get_all_financials() == {}

    def test_an_annual_for_an_unfinished_year_is_rejected(self, catalog) -> None:
        current = datetime.now(timezone.utc).year
        assert catalogue_financial_store.bulk_upsert_financials([self._row("AAA", current, 0, 999.0)]) == 0
        assert catalogue_snapshots.get_all_financials() == {}

    def test_a_completed_period_is_stored(self, catalog) -> None:
        last_fy = catalogue_periods._latest_complete_fiscal_year()
        assert catalogue_financial_store.bulk_replace_financials([self._row("AAA", last_fy, 0, 400.0)]) == 1
        assert catalogue_snapshots.get_all_financials()["AAA"]["revenue"] == 400.0
    ...
    def test_a_rejected_row_does_not_wipe_what_is_already_stored(self, catalog) -> None:
        ...
        last_fy = catalogue_periods._latest_complete_fiscal_year()
        catalogue_financial_store.bulk_replace_financials([self._row("AAA", last_fy, 0, 400.0)])

        catalogue_financial_store.bulk_replace_financials([self._row("AAA", last_fy + 1, 4, 999.0)])
    ...
    def test_the_predicate(self, year, quarter, future) -> None:
        assert catalogue_periods._is_future_period(year, quarter) is future
...
class TestSeveralPeriodsPerTicker:
    ...
    def test_both_periods_survive_the_replace(self, catalog) -> None:
        ...
        assert catalogue_financial_store.bulk_replace_financials(rows) == 2

        got = catalogue_snapshots.get_all_financials()["AAA"]
    ...
    def test_the_replace_still_clears_a_period_that_would_shadow_the_push(self, catalog) -> None:
        ...
        """
        last_fy = catalogue_periods._latest_complete_fiscal_year()
        catalogue_financial_store.bulk_upsert_financials([{"ticker": "AAA", "year": last_fy, "quarter": 3, "net_income": 7.0}])

        catalogue_financial_store.bulk_replace_financials([{"ticker": "AAA", "year": last_fy, "quarter": 0, "net_income": 400.0}])
    ...
    def test_the_replace_keeps_the_quarters_between_its_two_rows(self, catalog) -> None:
        ...
        catalogue_financial_store.bulk_upsert_financials(history)
    ...
    def test_an_annual_row_has_no_companion_of_its_own(self, catalog) -> None:
        ...
        assert catalogue_snapshots.get_all_financials()["AAA"]["annual"] is None

    def test_the_period_length_travels_with_the_row(self, catalog) -> None:
        last_fy = catalogue_periods._latest_complete_fiscal_year()
        catalogue_financial_store.bulk_replace_financials([{"ticker": "AAA", "year": last_fy, "quarter": 2, "net_income": 50.0}])
    ...
    def test_months_per_period(self, quarter, months) -> None:
        assert catalogue_periods._period_months(2020, quarter) == months
...
class TestUnitScale:
    ...
    def test_the_scale_factor_is_a_thousand(self) -> None:
        assert catalogue_fields.NSBU_THOUSANDS_UZS == 1000.0
    ...
    def test_every_money_field_is_scaled(self) -> None:
        ...
        assert set(catalogue_fields.FIN_MONEY_FIELDS) == {
        ...
        assert set(catalogue_fields.BALANCE_MONEY_KEYS) == {
    ...
    def test_absolute_ratio_sums_are_scaled_but_coefficients_are_not(self) -> None:
        assert set(catalogue_fields.RATIO_MONEY_FIELDS) == {"total_equity", "total_assets"}
        for coefficient in ("roe", "roa", "net_profit_margin", "debt_to_equity"):
            assert coefficient not in catalogue_fields.RATIO_MONEY_FIELDS
