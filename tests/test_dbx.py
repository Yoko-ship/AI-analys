...
class TestDialect:
    ...
    def test_null_safe_equality_survives_the_crossing(self):
        """`col IS ?` is how SQLite asks "same value, NULL included". PostgreSQL's
        IS takes only NULL/TRUE/FALSE, so a parameter after it is a SYNTAX ERROR —
        the statement never runs. That is what stopped every catalog sync after
        the cutover: `... AND year IS $4 ...` in the report upsert."""
        got = dbx.translate("SELECT id FROM r WHERE ticker=? AND year IS ? AND q=?", dbx.POSTGRES)

        assert got == "SELECT id FROM r WHERE ticker=%s AND year IS NOT DISTINCT FROM %s AND q=%s"

    def test_the_negated_form_too(self):
        got = dbx.translate("SELECT 1 FROM t WHERE a IS NOT ?", dbx.POSTGRES)

        assert got == "SELECT 1 FROM t WHERE a IS DISTINCT FROM %s"

    def test_a_test_against_a_keyword_is_not_a_parameter(self):
        """IS NULL / IS NOT NULL / IS TRUE are valid PostgreSQL as they stand;
        rewriting them would be the bug."""
        sql = "SELECT 1 FROM t WHERE a IS NULL AND b IS NOT NULL AND c IS TRUE"

        assert dbx.translate(sql, dbx.POSTGRES) == sql

    def test_the_same_words_inside_a_literal_are_untouched(self):
        got = dbx.translate("SELECT 'year IS ?' FROM t WHERE a IS ?", dbx.POSTGRES)

        assert got == "SELECT 'year IS ?' FROM t WHERE a IS NOT DISTINCT FROM %s"
