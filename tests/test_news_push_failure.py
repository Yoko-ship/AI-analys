...
import collectors.news.cli as collectors_news_cli
import collectors.news.cli as collectors_news_cli
...
class TestTheExitCode:
    def test_a_rejected_push_fails_the_run(self, cron_run):
        cron_run.setattr(collectors_news_runner, "run", lambda **k: {"fetched": 10, "pushed": 0,
                                                 "push_failed": True})
        with pytest.raises(SystemExit) as exc:
            collectors_news_cli.main()
        assert exc.value.code == 1

    def test_a_successful_push_exits_zero(self, cron_run):
        cron_run.setattr(collectors_news_runner, "run", lambda **k: {"fetched": 10, "pushed": 10,
                                                 "push_failed": False})
        collectors_news_cli.main()  # no SystemExit
    ...
    def test_an_empty_but_healthy_run_is_not_a_failure(self, cron_run):
        ...
        cron_run.setattr(collectors_news_runner, "run", lambda **k: {"fetched": 0, "pushed": 0,
                                                 "push_failed": False})
        collectors_news_cli.main()
...
class TestTheReportedFlag:
    def test_run_reports_the_push_verdict(self, monkeypatch):
        ...
        import inspect
        source = inspect.getsource(collectors_news_runner.run)
