...
def catalog(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(rc, "_json_get", lambda *args, **kwargs: [])
...
def test_sync_preserves_same_period_distinct_pdfs_and_is_idempotent(catalog, monkeypatch):
    ...
    monkeypatch.setattr(rc, "_fetch_main_results", lambda *args: (records, "bank"))
    for _ in range(2):
        result = rc.sync_company("TSTB", "Test bank", force=True, org_id="23")
...
def test_source_listing_filters_issuer_form_and_source_policy(catalog):
    ...
    assert rc._remember_ifrs_sources(ticker="TSTB", org_id="23", records=records) == 1
    c = store.connect()
    try:
        sources = c.execute("SELECT * FROM ingest_sources").fetchall()
