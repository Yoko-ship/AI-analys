...
def _no_filings(monkeypatch):
    ...
    monkeypatch.setattr(catalogue_snapshots, 'get_financials_series', lambda t, form="NSBU": {})


@pytest.fixture()
def client(monkeypatch):
    monkeypatch.setattr(catalogue_filings, 'get_company_index', lambda t: {"org_id": 666} if t == "UZTL" else {})
    monkeypatch.setattr(catalogue_facts, 'get_facts', lambda org, dataset=None: FACTS)
...
def test_only_the_verified_identities_carry_a_unit(monkeypatch):
    ...
    monkeypatch.setattr(catalogue_filings, 'get_company_index', lambda t: {"org_id": 1})
    monkeypatch.setattr(catalogue_facts, 'get_facts', lambda org, dataset=None: [
        _fact("roe", "2025", 19.5, unit="%"),
...
def test_the_money_field_list_is_the_contract():
    ...
    assert set(catalogue_fields.FACT_MONEY_FIELDS) == {
    ...
    for coefficient in ("roe", "roa", "net_margin", "current_ratio", "debt_to_equity"):
        assert coefficient not in catalogue_fields.FACT_MONEY_FIELDS
...
def test_a_zero_assets_placeholder_does_not_hide_a_real_income_statement(monkeypatch):
    ...
    """
    monkeypatch.setattr(catalogue_filings, 'get_company_index', lambda t: {"org_id": 633})
    monkeypatch.setattr(catalogue_facts, 'get_facts', lambda org, dataset=None: [
...
def test_a_junk_zero_assets_line_beside_real_equity_is_kept(monkeypatch):
    ...
    monkeypatch.setattr(catalogue_filings, 'get_company_index', lambda t: {"org_id": 11})
    monkeypatch.setattr(catalogue_facts, 'get_facts', lambda org, dataset=None: [
...
def test_the_newest_write_wins_a_repeated_period(monkeypatch):
    ...
    monkeypatch.setattr(catalogue_filings, 'get_company_index', lambda t: {"org_id": 666})
    monkeypatch.setattr(catalogue_facts, 'get_facts', lambda org, dataset=None: [
