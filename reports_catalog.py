...
# A financial table uses reader-facing names while the filing cache retains the
...
# These table values are legitimate calculations, but no filing has a row
# called "operating expenses" or "net margin".  A passport must say that,
...
def get_financial_value_passport(ticker: str, period: str, field: str,
    ...
    try:
        ...
        logger.exception("financial passport lookup failed for report %s", report_id)
        report_data = None
    if not report_data:
        return {
            "status": "NO_SOURCE_PASSPORT",
