...
def run_pending(limit=10):
    """Bounded worker. Source failures retry; data blockers await a new event."""
    import issuer_analysis_api as api
    from sector_report_service import sector_report
    stamp = now()
    # Validity windows activate automatically, including future-dated overrides.
    with connect() as connection:
        active_rules = connection.execute("SELECT version,ticker FROM sector_overrides WHERE valid_from<=? AND valid_to>=?",
                                          (stamp[:10], stamp[:10])).fetchall()
    for rule in active_rules:
        enqueue(rule["ticker"], "rule:" + rule["version"])
    with connect() as connection:
        # Recover interrupted workers after a lease expires.
        ...
        rows = connection.execute("SELECT * FROM sector_jobs WHERE state IN ('queued','retry') AND next_attempt<=? ORDER BY next_attempt LIMIT ?", (stamp, limit)).fetchall()
    for row in rows:
        with connect() as connection:
            claimed = connection.execute("UPDATE sector_jobs SET state='running',updated_at=? WHERE id=? AND state IN ('queued','retry')", (now(), row["id"])).rowcount
        if not claimed:
            continue
        error = None
        attempts = row["attempts"] + 1
        try:
            issuer = api._resolve_issuer(row["ticker"])
            reports = [sector_report(issuer, "nsbu", None, "separate", lang) for lang in ("ru", "uz")]
            retryable = any(r["status"] == "mapping_failed" for r in reports)
            state = "retry" if retryable and attempts < 4 else "incident" if retryable else "completed" if all(r["status"] == "available" for r in reports) else "blocked"
            if state != "completed":
        ...
        with connect() as connection:
            connection.execute("UPDATE sector_jobs SET state=?,attempts=?,next_attempt=?,error_code=?,updated_at=? WHERE id=?",
                               (state, attempts, next_attempt, error, now(), row["id"]))
    return len(rows)
