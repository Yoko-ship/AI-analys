...
test("sector report opens from the company card and exposes sourced formulas", async ({ page }) => {
  ...
  await expect(page.getByTestId("company-insight-card")).toContainText("выручка растёт, но прибыль снижается");
  await expect(page.getByTestId("company-insight-card")).toContainText("рост пока не повышает эффективность");
  ...
  await expect(dialog.locator(".company-insight-abstract")).toContainText("рост пока не повышает эффективность");
