...
Анализ динамики ключевых статей отчётности.

Представь изменения в табличном формате:
...
| Активы     | X XXX XXX      | X XXX XXX         | +/-X XXX   | +/-X,X% |
| Капитал    | X XXX XXX      | X XXX XXX         | +/-X XXX   | +/-X,X% |
| Обязательства | X XXX XXX   | X XXX XXX         | +/-X XXX   | +/-X,X% |

Интерпретация: развёрнутый анализ причин изменений. Какие факторы обусловили рост/падение ключевых статей. Как динамика соотносится с отраслевыми трендами.

[ВЕРТИКАЛЬНЫЙ_АНАЛИЗ]
Структурный анализ баланса и отчёта о прибылях и убытках.

СТРУКТУРА АКТИВОВ (% от итога):
— Денежные средства и эквиваленты: X,X%
...
СТРУКТУРА ПАССИВОВ:
— Собственный капитал: X,X% — [интерпретация уровня автономии]
— Долгосрочные обязательства: X,X%
— Краткосрочные обязательства: X,X%

СТРУКТУРА ДОХОДОВ:
— Валовая маржа: X,X% (Валовая прибыль / Выручка)
— Операционная маржа: X,X% (EBIT / Выручка)
— Чистая маржа: X,X% (Чистая прибыль / Выручка)

Вывод: оценка качества структуры — сбалансированная / требует внимания / рискованная. Обоснуй заключение.

[КОЭФФИЦИЕНТНЫЙ_АНАЛИЗ]
Расчёт и интерпретация ключевых финансовых коэффициентов.

ЛИКВИДНОСТЬ:
• Текущая ликвидность (Current Ratio) = Оборотные активы / Краткосрочные обязательства = X,XX
  Формула: {current_assets} / {current_liabilities} = X,XX
  Интерпретация: [норма для UZ: 1,5-2,5]

• Быстрая ликвидность (Quick Ratio) = (Оборотные активы − Запасы) / Краткосрочные обязательства = X,XX
  Интерпретация: [норма для UZ: 0,8-1,5]

РЕНТАБЕЛЬНОСТЬ:
• ROE (Return on Equity) = Чистая прибыль / Собственный капитал × 100% = X,X%
  Формула: {net_income} / {equity} × 100% = X,X%
  Интерпретация: на каждые 100 сум капитала акционеров компания генерирует X сум прибыли [норма для UZ: >12%]

• ROA (Return on Assets) = Чистая прибыль / Активы × 100% = X,X%
  Интерпретация: эффективность использования всех активов [норма для UZ: >5%]

• ROS (Return on Sales) = Чистая прибыль / Выручка × 100% = X,X%
  Интерпретация: маржинальность бизнеса [норма для UZ: зависит от отрасли]

ДОЛГОВАЯ НАГРУЗКА:
• Debt Ratio = Обязательства / Активы = X,X%
  Интерпретация: доля заёмного финансирования [безопасный уровень: <60%]

• Debt/Equity = Обязательства / Собственный капитал = X,XX
  Интерпретация: соотношение долга к собственным средствам [норма: <1,5]

• Interest Coverage = EBIT / Процентные расходы = X,XX раз
