...
КАЖДАЯ секция должна начинаться со СТРУКТУРИРОВАННОГО TL;DR-БЛОКА в строго таком формате:
...
ЖЁСТКИЕ ПРАВИЛА ДЛЯ TL;DR-БЛОКА:
1. В TL;DR-блоке (Тон/Плюсы/Минусы/Для тебя) ЗАПРЕЩЕНЫ голые аббревиатуры: ROE, ROA, EBIT,
...
def run_analysis(company_name: str, company_profile: str, annual_data: list,
    ...
    instructions = (
        ...
        "Сразу после метки секции — СТРУКТУРИРОВАННЫЙ TL;DR-блок строго в формате:\n"
        "    TL;DR\n"
        "    Тон: <позитивный | умеренный | тревожный | критичный | нет_данных>\n"
        ...
        "🚨 КРИТИЧНО ВАЖНО: каждая строка TL;DR-блока — на ОТДЕЛЬНОЙ строке, с реальным "
        "переводом строки (символ \\n). Не сжимай TL;DR в один абзац. Не пиши "
        "'TL;DR Тон: ... Плюсы: - ... Минусы: - ... Для тебя: ...' одной строкой. "
        ...
        "В блоке TL;DR (Тон/Плюсы/Минусы/Для тебя) КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНЫ голые "
        ...
        "В секции [ИТОГ] после TL;DR-блока обязателен ХЕРО-АБЗАЦ для новичка: 4-6 предложений "
